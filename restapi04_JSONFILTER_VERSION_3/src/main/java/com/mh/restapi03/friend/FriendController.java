...
public class FriendController {
    ...
    public ResponseEntity<FriendDto> getFriendByNickname(@PathVariable String nickname) {
        FriendDto friend = friendService.getFriendByNickname(nickname);
        if (friend == null) {
    ...
    @PostMapping()
    public ResponseEntity<FriendDto> addFriend(@RequestBody @Valid FriendDto friendDto) {
        FriendDto dbfriend = friendService.addFriend(friendDto);
    ...
    public ResponseEntity<FriendDto> addFriendApproval(@RequestBody FriendDto friendDto) {
        try {
            FriendDto friend = friendService.addFriendApproval(friendDto);
    ...
    public ResponseEntity<FriendDto> updateFriendApproval(@PathVariable int fidx, @RequestBody FriendDto friendDto) {
        try {
            FriendDto friend = friendService.updateFriendApproval(fidx, friendDto);
            if (friend == null) {
                return ResponseEntity.notFound().build();
            }
            return ResponseEntity.ok(friend);
